...
npm test                # 280 個單元測試，改完務必跑
npm run typecheck       # tsc --noEmit
npm run sim 16 all      # ★ 難度曲線：主線十二關各 16 局，印中位數／比例／與該關 arc 目標的偏差
...
npm run ai              # AI 代管成績：強 AI 跑主線十二關，印通關率與陣亡波次中位數
...
現況（`npm run sim 16 all`，括號是抵達比例）：
黃巾 12／12（1.00）・董卓 16／18（0.89）・巨鹿 23／30（0.77）・官渡 18／24（0.75）・
赤壁 19／30（0.63）・五丈原 25／40（0.63）・襄陽 17／32（0.53）・漢中 15／32（0.47）・
洛陽 20／40（0.50）・合肥 15／36（0.42）・虎牢關 12／30（0.40）・許昌 14／40（0.35）。
**驗收看的是「比例一路遞減」**——那一欄就是難度曲線本身。單關偏了就調那一關的 `arc`；
整條曲線一起偏移＝玩家端的平衡動了，那時改 `tools/autobalance.ts` 的 `DEATH_REF`，不要整條一起改。
⚠ **「偏差」那一欄對 `arc` 不敏感**：實際比例 ≈ k/arc、預期比例 = 20/arc，兩者同時吃 `arc`，
所以偏差量的是 k（這一關的地圖／生命／字池讓傻 AI 多撐了幾成）。要修的是**排序**。
⚠ `arc` 不是照關卡順序等差就會對：生命數與字池也算難度（2 條命的五丈原用的弧比襄陽短）。
⚠ 漢中 0.47／洛陽 0.50 的先後在 16 局取樣下屬於雜訊（洛陽是雙峰分佈：一部分種子前 5 波就崩、
一部分撐到 20+），不值得再疊 `arc` 去追。
⚠ **歷史**：`arc` 曾經不存在（弧長寫死 `WAVE_REF`），於是每關都走完整條弧、傻 AI 在每一關都死在正中間
——難度一樣平，而 12 波的教學關被壓成每波血量 ×1.99，是全遊戲最陡的一段。別再走回去。
**無盡變體**（`endless_<key>`，`maxWave: Infinity`）沒有總波數，改走絕對波次曲線，目標 `WAVE_REF/2 = 20`：
黃巾 22・巨鹿 21・洛陽 19。

`npm run econ` 是經濟儀表板。**經濟目標：一波只夠征兵 1～2 次**（現況 1.25）。
...
8. **敵方光環一律要有疊加上限，而且治療者之間不互相治療** — 見下面的「卡波」段。
   `Enemy.def` 與 `Enemy.speed` 因此是**每幀重算的衍生值**，不是可以長期寫入的狀態。

完整清單與陷阱：[docs/llm-wiki/04-invariants.md](docs/llm-wiki/04-invariants.md)

## 卡波：波次一定要收得掉

**卡波＝整局停在同一波動不了**：場上那批敵人清不掉、生命一點沒少、時間就停在那裡。
它需要兩件事同時成立，而且會互相加乘——**打不死**（一群妖道互相回血）＋
**推不動**（擊退／定身把敵人壓在原地，於是它們也走不到大營結束這一波）。

修法分三層，缺一不可（`sim/step.ts`、`data/enemies.ts`）：

1. **治療者之間不互相治療**。N 隻妖道互奶時每隻收到 (N−1)×hps，**隨數量平方成長**，
   6 隻疊在一起就是每秒回 25%——任何輸出都追不上。妖道本身脆，沒人奶它就殺得掉。
2. **光環疊加有上限**：`HEAL_CAP_HPS` 0.12／`DEF_ADD_CAP` 60／`SPEED_AURA_CAP` 1.8，
   外加敵表的 `maxShare`（單一敵種在一波裡的數量佔比上限，妖道 0.12、旗賊 0.15）。
3. **督戰 `stepFrenzy`——與成因無關的最後保證**。只要玩家的擊退夠強，光靠「推不動」
   就能無限拖住，跟治療無關，所以一定要有這一層。判定看的是「**有沒有進展**」
   （最前方進度／擊殺數／場上總血量，三個水位任一動了就重新計時），
   連續 8 秒都沒動就在 6 秒內把 `frenzy` 拉到 1：回血歸零、敵速 ×3、定身與擊退失效。

⚠ `frenzy` 在一波之內**只升不降**。曾經讓它「一有進展就回退」，結果是擊退鎖進入穩定震盪、
波次照樣結束不了。⚠ 它是**保險不是難度旋鈕**，正常對局（哪怕很慢）永遠碰不到它——
`stall.test.ts` 兩面都有測試：僵局一定會被打破、正常推進與「控住磨血」一定不會誤觸發。
細節見 [modules/05](docs/llm-wiki/modules/05-economy-and-waves.md) 的「卡波與督戰」。

## 戰場特性（`LevelDef.mods`）

難度不再只有「血量長多快」一條軸。關卡可以宣告 `mods` 改一條規則，
四個旋鈕都是**中性預設值**（省略＝舊行為，沿用 `Perks` 的分層慣例）：
`bossEvery`（BOSS 密度）／`spawnGap`（出怪間隔）／`enemySpeedMul`（行軍速度）／`rangeMul`（我方射程）。
玩家看到的說明由 `modTags()` **單一推導**，不要在關卡資料裡另外手寫一份。
⚠ `arc` 換算出的預期比例不知道 `mods` 存在——加了一定要重跑 `npm run sim`。
...
內容量：**71 字／69 武將（26 姓名配方＋43 字組合）／13 羈絆／33 個主動技／5 個組合技／24 種敵人（11 一般＋13 BOSS）／12 關**
（4 關固定地圖 + 8 關隨機地形，每關通關後另開放一個**無盡變體**）。每關宣告 `bias`（偏好的敵人特徵），加權該類敵人與 BOSS 的出現率，
並自動推導出選單上的「建議帶」標籤——見 `data/enemies.ts` 的 `TRAIT_COUNTERS`；
終盤三關（合肥／虎牢關／許昌）另帶**戰場特性** `mods`，各動一條與血量無關的規則。
...
無盡模式（12 關各一個推導變體，`maxWave: Infinity`，見 `data/levels/index.ts` 末段；
...
實測每輪 1.21→0.49ms（巨鹿）／2.17→0.73ms（洛陽），而 `npm run ai` 的各關中位數完全不變。
`npm run perf` 是它的量表。
...
現況各關波數中位數與通關率：黃巾 12（88%）・董卓 18（88%）・巨鹿 30（38%）・官渡 21（13%）・赤壁 30（38%）・
五丈原 40（38%）・襄陽 28・漢中 25・洛陽 30・合肥 27・虎牢關 19・許昌 25（後六關 0%）
（`npm run ai` 八局／關，總通關率 25%，耗時 ≈99s）。

...
手寫 `map` 時，`parseMap` 只保證「S 走得到 C」，**不保證每一格路都在敵人走的那條路上**。
黃巾之亂原本把 `C` 放在右下角，而最後一段路是往左走的，於是最後一列往左的 8 格
變成沒有出口的死路：玩家看到一條走到底卻什麼都沒有的路，敵人卻在半路轉下去進營。
這種錯不會拋錯、只會安靜地誤導人，所以 `mapgen.test.ts` 有一條「沒有走不到的路格」把關。
畫蛇形地圖的規則：**大營要放在最後一段路走完的那一端**（見 `julu` 的排法）。
...
`npm test` 目前 280 個測試（**請以 `npm test` 的輸出為準，不要用 grep 數 `it(`**——
...
- `mapgen.test.ts` — 180 張隨機地圖的連通性、走廊不變量（可走格數 == 路徑長度）、可放置空地下限、種子可重現、
  **固定地圖沒有走不到的路格**（黃巾曾把 `C` 放在錯的角落 → 最後一列 8 格死路）、
  **九關的 `arc` 逐關不遞減**且教學關的每波血量成長遠比舊版平緩（難度曲線守護）
...
- `enemies-ext.test.ts` — 敵表完整性（key 不重複、BOSS 全部 ccImmune、**13 種 BOSS 的機制指紋互不相同**
...
- `core.test.ts`（波次曲線段）— **血量指數吃「相對進度」** `wave × arc / maxWave`：走完同樣比例的弧
  得到同樣血量、`arc` 越長越硬、**短關卡配短弧與長關卡一樣平緩**（`maxWave` 不再單獨決定難度）、
  省略時等同舊的絕對波次公式、`buildWave` 有把 `maxWave` 與 `arc` 都傳下去
...
- `endless.test.ts` — **無盡模式**：每一關的無盡變體都由原關推導（數值／地圖／`bias` 一致）、
  不在 `LEVEL_ORDER` 裡（解鎖鏈與每日輪替不受影響）、key helper 可來回轉換、
  **`maxWave = Infinity` 時血量改走絕對波次**（漏了會讓敵人永遠停在第 0 波強度）、
  單波敵數上限且有限關卡碰不到、**清完波只會進下一波、永遠不會 `won`**（附有限關卡的對照組）、
  生命歸零照樣落敗（守 `checkWaveEnd` 不覆寫 `lost` 那條）、快照還原後仍然是無盡
- `stall.test.ts` — **卡波（波次一定要收得掉）**：治療者之間不互相治療（含妖道首不被雜兵妖道奶）、
  妖道仍會治療一般敵人、多隻疊加時每秒回血總量有上限、單波妖道數量有上限（`maxShare`）、
  加防／加速光環同樣有上限且**不會逐幀累積**、沒有光環來源時攻防速維持敵表原值；
  督戰：**無限擊退＋定身也不會讓敵人永遠留在場上**、使用者回報的「一整包妖道＋擊退鎖」整波仍會收掉、
  一波之內只升不降且過波歸零；**反面**：正常推進的慢波次與「控住高血敵人慢慢磨」都不會誤觸發
- `level-mods.test.ts` — **戰場特性**：沒宣告 `mods` 的關卡行為與加這個系統之前完全一致（中性預設值契約）、
  `bossEvery` 改變 BOSS 波、`spawnGap` 只動時間軸不動組成（同種子仍是同一局）、
  `rangeMul`／`enemySpeedMul` 各自與對應的 `perks` 相乘、每個宣告 `mods` 的關卡都推導得出 `modTags` 說明、
  無盡變體沿用原關的 `mods`

...
- 12 關，其中 8 關**每局隨機生成地形**（由構造保證沒有死路，見 `sim/mapgen.ts`）。
- 終盤三關另帶**戰場特性**（`LevelDef.mods`）：出怪節奏／BOSS 密度／我方射程各動一條規則。
...
- 內容量：**71 字／69 武將（26 姓名配方＋43 字組合）／13 羈絆／33 主動技／5 組合技／24 種敵人（11 一般＋13 BOSS）／12 關**。
...
src/
  data/        glyphs.ts      字表（71）
               ...
               bonds.ts       羈絆（13）
               enemies.ts     敵表（24：11 一般 + 13 BOSS）+ 特徵對照表
               levels/index.ts 關卡（12，各帶 bias 敵人偏好與 mods 戰場特性）
               upgrades.ts    兵書：4 種數值養成
               shop.ts        商城：16 種可升級被動道具 → Perks
               loadout.ts     編隊：手動挑選字池內容
               daily.ts       每日挑戰：日期 → 關卡與種子（純函式）
               levels/index.ts 末段  無盡變體（由 12 關推導，maxWave: Infinity）

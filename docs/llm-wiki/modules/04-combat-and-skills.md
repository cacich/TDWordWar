...
> | 檔案 | 規模 | 職責 |
> |---|---|---|
> | `src/sim/combat.ts` | 342 行 | 索敵、射程、傷害公式、控場狀態、特效佇列（`stepCombat` / `stepEffects`） |
...
>
> **下游使用者**：
> - `sim/step.ts:31-33` 每 tick 依序呼叫 `stepCombat` → `stepSkills` → `stepBondSkills`
> - `sim/state.ts:374` 用 `effectiveRange()` 寫 `Unit.range`；`state.ts:364` 用 `computeBonds()`；`state.ts:303,396` 用 `SKILLS[]` 判斷「技能有沒有實作」
...
| `damageEnemy(state, e, dmg)` | `combat.ts:151` | 只扣血 + 死亡結算（給糧 `bounty × perks.bountyMul`、`stats.kills`、`emit('kill')`、飄字） | **沒有施放者**的持續／環境傷害：灼燒（`step.ts:109`）、流星火雨（`step.ts:59`） |

- `dealDamage` **必經** `damageEnemy`，所以擊殺獎勵只寫在一個地方；灼燒擊死也會給糧（`m3.test.ts:112`）。
- 傳給 `damageEnemy` 的數字是**最終傷害**，不再過 `mitigate()`。組合技要「無施放者但仍吃防禦」時用
  `skills.ts:63` 的 `flatDamage()`（= `damageEnemy(mitigate(raw, def))`）。
- `applyOnHit = false` 用來避免二次施加狀態：連鎖雷（`combat.ts:287`）與 `crowd` 原型（`skills.ts:92`）都靠它，
...
export function canHit(u: Unit, e: Enemy): boolean {
  ...
  return u.baseRange >= ANTI_AIR_RANGE   // ANTI_AIR_RANGE = 2.0（data/enemies.ts:20）
}
```
...
- `shape`（`combat.ts:277-331`）：`single`（可帶 `onHit.chain`，附近 1.8 內額外 chain 個目標、傷害 60%）／
  `pierce`（`|e.dist − target.dist| <= 1.3`，最多 3 名）／`splash`（命中點 1.3 格內全體）。
  後兩者傷害額外乘 `perks.splashMul`（烽火連城），且**每個命中目標都會吃到 onHit**。
- 已組成武將的字牌**跳過攻擊**（`combat.ts:244`）——由武將那一層代表出手，否則傷害雙算。

### 6. 控場狀態

狀態存在 `Enemy` 上，全部是「剩餘秒數」（`types.ts:211-217`），由 `step.ts:100` 的 `stepStatuses` 倒數，
`applyStatus`（`combat.ts:208`）一律用 `Math.max` 覆寫（**取較長者，不累加**）。

| 狀態 | 欄位 | 生效處 | `ccImmune` 擋？ |
|---|---|---|---|
| 減速（固定 50%，`SLOW_FACTOR`） | `slow` | `step.ts:120` | ✗ 不擋 |
| 灼燒（每秒 `baseAtk × burn.mul`） | `burnT` / `burnDps` | `step.ts:107-111` → `damageEnemy` | ✗ 不擋 |
| 易傷（`VULN_MUL = 1.3`） | `vuln` | `dealDamage`（`combat.ts:185`） | ✗ 不擋 |
| 定身 | `stun` | `step.ts:119`（不前進） | **✓ 擋** |
| 擊退（`dist -= knock`） | `dist` | `combat.ts:217` | **✓ 擋**（連特效環也不畫，`combat.ts:219`） |
| 連鎖 | — | `stepCombat` 內即時處理，不是狀態 | — |

`ccImmune` 只在 `combat.ts:215,219` 出現，僅包住 `stunDur` 與 `knock`；
BOSS（`data/enemies.ts:12` 的 `boss`／賊將）仍然吃灼燒、減速、易傷。這是刻意的平衡（控場流不該癱瘓 BOSS，也不該完全無效）。
...
| `healLife` | `(n)` | 玩家生命 | `lives >= maxLives`（`m3.test.ts:199`） |
...
同一哲學也在攻擊上：`stepCombat` 找不到目標時把 `u.cd = 0`（`combat.ts:251`），下一 tick 立刻可打。
...
多個羈絆**相乘**（`bonds.ts:41-43`）。`state.cdMul = bonds.cdMul × perks.cdMul`（`state.ts:367`）。
...
└─ step.ts:15 stepGame
   ...
   └─ cleanupDead / checkWaveEnd
```

倍率的重算走另一條路（**不在每 tick**）：

```
actions.ts 任一操作 → recalcUnits(state)（state.ts:357）
...
2. **爆擊的 RNG 慣例是「短路、不抽樣」，跟另外兩處相反。**
   ...
   ```
   但 `step.ts:129`（回魂旗漏怪防護）與 `actions.ts:49`（精兵符）是 **`state.rng()` 無論如何都抽**。
...
3. **`applyStatus` 的第 4 參數 `baseAtk` 只餵灼燒**（`combat.ts:213`：`burnDps = baseAtk × burn.mul`）。
...
7. **`Effect` 只是視覺，`pushEffect` 超過 240 筆直接丟棄**（`combat.ts:235`）。不要把任何機制資訊塞進 `state.effects`，它會被丟。
   `ring` 用 `toX − fromX` 偷渡半徑給 renderer（`skills.ts:38`），改 `Effect` 結構時注意這個約定。

8. **音效與粒子一律走 `emit()`**（`attack` / `kill` / `skill` / `combo`），`sim/` 內不可碰 Audio／canvas。事件佇列上限 64（`types.ts:266`）。

9. `stepCombat` 用 `u.atk <= 0 || u.aps <= 0` 過濾光環／經濟字（`combat.ts:245`）；`effectiveRange` 另外用 `baseRange <= 0` 短路。兩個條件都要成立才算「完全不攻擊」。
...
| 誰能打飛行 | `data/enemies.ts:20` 的 `ANTI_AIR_RANGE`，或個別字的 `range` | 判定用 `baseRange`；調 `RANGE_MUL` **不會**改變對空名單（刻意） |
...
| 新增一種控場狀態 | 要改六處（`OnHit` 型別、`Enemy` 欄位、`spawnDue` 初始化、`applyStatus`、`stepStatuses`、生效點） | 清單見 [../03-change-recipes.md](../03-change-recipes.md) §10b；漏一處會安靜失效 |
...
| BOSS 免疫的範圍 | `combat.ts:215,219` | 目前只擋 stun/knock。要擋灼燒等就得在 `applyStatus` 前段也包 `if (!e.ccImmune)`，並同步改 `data/enemies.ts:12` 的 `desc` 與 `m3.test.ts:132` |

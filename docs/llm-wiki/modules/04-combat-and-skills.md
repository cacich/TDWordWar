...
> | 檔案 | 規模 | 職責 |
> |---|---|---|
> | `src/sim/combat.ts` | 354 行 | 索敵、射程、傷害公式、控場狀態與三種免疫、特效佇列（`stepCombat` / `stepEffects`） |
...
>
> **下游使用者**：
> - `sim/step.ts:33-35` 每 tick 依序呼叫 `stepCombat` → `stepSkills` → `stepBondSkills`
> - `sim/state.ts:426` 用 `effectiveRange()` 寫 `Unit.range`；`state.ts:417` 用 `computeBonds()`；`state.ts:355,432` 用 `SKILLS[]` 判斷「技能有沒有實作」
...
| `damageEnemy(state, e, dmg)` | `combat.ts:151` | 只扣血 + 死亡結算（給糧 `bounty × perks.bountyMul`、`stats.kills`、`emit('kill')`、飄字） | **沒有施放者**的持續／環境傷害：灼燒（`step.ts:284`）、流星火雨（`step.ts:61`） |
...
- `applyOnHit = false` 用來避免二次施加狀態：連鎖雷（`combat.ts:299`）與 `crowd` 原型（`skills.ts:92`）都靠它，
...
再由 `recalcUnits`（`state.ts:425`）乘上局外道具 `perks.rangeMul`（精工兵器）寫進 `Unit.range`。
...
export function canHit(u: Unit, e: Enemy): boolean {
  ...
  return u.baseRange >= ANTI_AIR_RANGE   // ANTI_AIR_RANGE = 2.0（data/enemies.ts:199）
}
```
...
- `shape`（`combat.ts:289-343`）：`single`（可帶 `onHit.chain`，附近 1.8 內額外 chain 個目標、傷害 60%）／
  `pierce`（`|e.dist − target.dist| <= 1.3`，最多 3 名）／`splash`（命中點 1.3 格內全體）。
  後兩者傷害額外乘 `perks.splashMul`（烽火連城），且**每個命中目標都會吃到 onHit**。
- 已組成武將的字牌**跳過攻擊**（`combat.ts:256`）——由武將那一層代表出手，否則傷害雙算。

### 6. 控場狀態與三種免疫

狀態存在 `Enemy` 上，全部是「剩餘秒數」（`types.ts:284-290`），由 `step.ts:275-288` 的 `stepStatuses` 倒數，
`applyStatus`（`combat.ts:219-244`）一律用 `Math.max` 覆寫（**取較長者，不累加**）。
...
| 狀態 | 欄位 | 生效處 | 被誰擋掉 |
|---|---|---|---|
| 減速（固定 50%，`SLOW_FACTOR`） | `slow` | `step.ts:300` | **`slowImmune`**（`combat.ts:220`） |
| 灼燒（每秒 `baseAtk × burn.mul`） | `burnT` / `burnDps` | `step.ts:282-286` → `damageEnemy` | **`burnImmune`**（`combat.ts:222`） |
| 易傷（`VULN_MUL = 1.3`） | `vuln` | `dealDamage`（`combat.ts:185`） | **沒有任何免疫擋得住**（`combat.ts:221`） |
| 定身 | `stun` | `step.ts:299`（不前進） | `ccImmune`（`combat.ts:227-228`） |
| 擊退（`dist -= knock`） | `dist` | `combat.ts:229` | `ccImmune`（連特效環也不畫，`combat.ts:231`） |
...
⚠ 免疫是在 `makeEnemy` 從 `EnemyDef` **複製**進 `Enemy` 的（`step.ts:87-89`），不是每次回查敵表。
...
⚠ 另一個繞過點：`stepMeteor` 直接寫 `e.burnT` / `e.burnDps`（`step.ts:63-64`），**沒有經過 `applyStatus`，
...
- 沒註冊 → `state.ts:354,432` 讓 `skillCdMax = 0` → `stepSkills` 直接 `continue`，**技能永遠不觸發，但圖鑑／面板仍顯示描述文字**（安靜失效）。
...
同一哲學也在攻擊上：`stepCombat` 找不到目標時把 `u.cd = 0`（`combat.ts:263`），下一 tick 立刻可打。
...
多個羈絆**相乘**（`bonds.ts:41-43`）。`state.cdMul = bonds.cdMul × perks.cdMul`（`state.ts:419`）。
...
```
actions.ts 任一操作 → recalcUnits(state)（state.ts:409）
...
2. **爆擊的 RNG 慣例是「短路、不抽樣」，跟另外兩處相反。**
   ...
   ```
   但 `step.ts:308`（回魂旗漏怪防護）與 `actions.ts:49`（精兵符）是 **`state.rng()` 無論如何都抽**。
...
3. **`applyStatus` 的第 4 參數 `baseAtk` 只餵灼燒**（`combat.ts:224`：`burnDps = baseAtk × burn.mul`）。
...
7. **`Effect` 只是視覺，`pushEffect` 超過 240 筆直接丟棄**（`combat.ts:248`）。不要把任何機制資訊塞進 `state.effects`，它會被丟。
   `ring` 用 `toX − fromX` 偷渡半徑給 renderer（`skills.ts:38`），改 `Effect` 結構時注意這個約定。

8. **音效與粒子一律走 `emit()`**（`attack` / `kill` / `skill` / `combo`），`sim/` 內不可碰 Audio／canvas。事件佇列上限 64（`types.ts:341`）。
   ⚠ 敵方的死亡分裂與回血**刻意不 emit 任何事件**（分裂一次可能生 6 隻、回血每幀發生，會把佇列與音效淹掉）。

9. `stepCombat` 用 `u.atk <= 0 || u.aps <= 0` 過濾光環／經濟字（`combat.ts:257`）；`effectiveRange` 另外用 `baseRange <= 0` 短路。兩個條件都要成立才算「完全不攻擊」。
...
| 誰能打飛行 | `data/enemies.ts:199` 的 `ANTI_AIR_RANGE`，或個別字的 `range` | 判定用 `baseRange`；調 `RANGE_MUL` **不會**改變對空名單（刻意）。目前飛行敵人有飛賊與飛將兩種 |
...
| 新增第四種免疫 | `types.ts` 的 `EnemyDef` ＋ `Enemy` ＋ `step.ts:87-89` 的 `makeEnemy` ＋ `applyStatus`（`combat.ts:219-244`） | 漏掉 `makeEnemy` 那一行會靜默失效。**易傷刻意保持不可免疫**，別順手補上去 |

...
> | `src/ui/screens.ts` | 560 行 | 七個全螢幕畫面（menu／codex／forge／shop／loadout／achieve／dev） |
> | `src/ui/wish.ts` | 85 行 | 心願單挑選面板（列本局字池，不是全字表） |
> | `src/input/pointer.ts` | 281 行 | Pointer Events：拖放、點選待放置、鏟除、疊合、交換 |
> | `index.html` | 186 行 | 全部 DOM 節點（HUD 與七個 `.screen` 都寫死在這裡，JS 只填內容） |
...
`app.ts:5-32` 是全專案唯一同時 import `core/` + `data/` + `sim/` + `render/` + `ui/` + `input/` 的地方。
其他模組**彼此不直接依賴**：
...
`sim/` 只呼叫 `emit(state, ev)`（`sim/events.ts:9`）把純資料塞進 `state.events`，上限 `MAX_EVENTS = 64`（`sim/types.ts:297`），超量直接丟棄。
app 每幀 `drainEvents()`（`app.ts:216-278`）翻譯成音效與粒子，最後 `evs.length = 0`（`app.ts:277`）清空。

**目前 12 種 `SimEvent`（定義在 `sim/types.ts:282-294`）與各自的呈現**：

| 事件 | 音效 | 粒子／其他 | 程式碼 |
|---|---|---|---|
| `place {char}` | `place`（節流 0.04s） | — | app.ts:222 |
| `merge {char, level}` | `merge` | `ps.merge(格心, qualityColor(level))`；座標靠 `char` 反查場上第一個同字 glyph | app.ts:225-233 |
| `combine {name, tier, cells}` | `tier` 為 `legendary`/`mythic` → `combineBig`，否則 `combine` | `ps.combine(cells 形心, tier)`（粒子數與顏色依 tier） | app.ts:234-237 |
| `dissolve {name}` | `dissolve` | — | app.ts:238 |
| `attack {fx, x, y}` | `attackSfx(fx)`，音量 0.9 | 無粒子——攻擊的視覺走 `state.effects` + `render/fx.ts` | app.ts:241-243 |
| `kill {x, y, bounty}` | `kill`，音量 0.8（節流 0.05s） | `ps.kill(x, y)` 墨點 | app.ts:244-247 |
| `skill {name, x, y}` | `skill` | `ps.skill(x, y)` 金色火花 | app.ts:248-251 |
| `combo {name}` | `combo` | `ps.combo(大營格心)` + `hud.toast('組合技：…')` | app.ts:252-258 |
| `leak` | `leak` | `ps.leak(大營格心)` 紅色潑濺 | app.ts:259-265 |
| `waveClear {wave}` | `wave` | — | app.ts:266-267 |
| `won` | `win` | — | app.ts:269 |
| `lost` | `lose` | — | app.ts:272 |

`attackSfx()`（`app.ts:516-531`）把 10 種 `FxKind` 收斂成 5 個攻擊音：`blade`/`thrust` → `attackBlade`、`arrow` → `attackArrow`、`fire`/`venom` → `attackFire`、`bolt` → `attackBolt`、其餘（`gale`/`plan`/`charge`/`none`）→ `attackSoft`。
...
| `ScreensHost` | `ui/screens.ts:31-54` | 局外事務：`getMeta` / `achieveProgress` / `startLevel` / `show` / `buyUpgrade` / `buyItem` / 三個 loadout 方法 / 七個 dev 方法。**完全不碰 `GameState`**——成就畫面要的進度是 app 層算好成 `Record<key, number>` 再交過來，就是為了守住這個性質 |

`App` 一次實作三個（`app.ts:37`）。`WishHost`（`ui/wish.ts:10-14`）是第四個小介面，只要 `getState` / `toggleWish` / `closeWishPanel`。

### 5. `--ui` 等比縮放

`syncUiScale()`（`app.ts:110-118`）：

```
ui = clamp(9.5, 18, min(#app 寬 / 27, #app 高 / 56))   // app.ts:114
```
...
`#app.dataset.compact = 寬 < 320`（`app.ts:117`）→ `style.css:78-80` 在極窄視窗隱藏 `#level-name`（關卡名是遊戲中最不需要的資訊，選單就看得到）。想加更多 compact 隱藏項就照這條 selector 疊。

觸發時機（`app.ts:73-83`）：`window resize`、`orientationchange`、canvas 的 `ResizeObserver`、`#app` 的 `ResizeObserver`。canvas 的 CSS 尺寸會因浮層／軟鍵盤／瀏覽器工具列改變，**光靠 window resize 會漏**。

## 主要流程

### 啟動

`main.ts:11` `new App(canvas, loadMeta())` → `app.ts:58-104` 建 state / renderer / hud / screens / wishPanel / input / audio → 掛手勢解鎖與 resize 監聽 → `startLoop(...)` → `this.show('menu')`（`app.ts:103`，開場一定停在選單）。`main.ts:12` 才註冊 SW，`main.ts:21-40` 掛 `__dev`。
...
- `render` 收到的 `alpha`（插值係數）**目前沒被使用**：`app.ts:90-100` 的 render callback 自己用 `performance.now()` 算 `frameDt`（同樣 clamp 0.25s，`app.ts:92`）餵給粒子與 toast 計時。沒有做位置插值——格狀塔防的視覺誤差看不出來。

render callback 的固定順序（`app.ts:90-100`）：寫入 `renderer.view.selectedCell` → `drainEvents()` → `renderer.draw(state, frameDt)` → `hud.update(frameDt)` → `wishPanel.update()`（僅開啟時）→ `syncProgress(frameDt)`。
**`drainEvents()` 必須在 `draw()` 之前**：粒子要在同一幀就被 `particles.step()` 推進與畫出。

### 畫面切換與凍結

`App.show(screen)`（`app.ts:398-404`）= `screens.show(screen)` + `loop.setPaused(screen !== null)`。
`Screens.show()`（`ui/screens.ts:134-200`）把七個 `.screen` 的 `hidden` 全設好（一次只顯示一個）再呼叫對應的 render 方法。`screen === null` 代表回對局。
雙重保險：`step` callback 另外檢查 `this.screens.visible`（`app.ts:87`），`syncProgress` 也在畫面開著時直接 return（`app.ts:126`）。
`dev` 畫面的入口是彩蛋：選單標題 2.5 秒內連點 7 下（`ui/screens.ts:123-132`）。
...
- `hintKind`（`renderer.ts:61`，建於 234-238）：消費 `state.hintCells`（`sim/types.ts:435-446`），一格同時可升級與可湊將時 **upgrade 優先**（更直接可做）
...
`performance.now()` 只出現在呈現層（`renderer.ts:310`、`app.ts:50/88`、`screens.ts:124`、`loop.ts` 內）。**`sim/` 內禁用**。

### 進度與存檔：`syncProgress()`（`app.ts:125-190`）

每幀做四件事，全在 app 層而**不在 sim 層——這樣 `sim/` 完全不知道 localStorage 與 `MetaProgress` 的存在**，`npm run sim` 也就不會污染玩家存檔：

1. 掃 `state.hand` 與 `state.units` 寫入 `meta.seenGlyphs` / `seenGenerals`（`app.ts:128-144`）
2. `meta.best[levelKey]`，**只在 `wave > 1` 才記**（`app.ts:149`），否則一進關卡就顯示「最佳 1 波」
3. `phase === 'won'` 時補 `meta.cleared`（`app.ts:153-156`）
4. 聲望結算：`renownPaid` 旗標保證一局只結一次（`app.ts:159-181`），數值由 `renownFor()`（`sim/state.ts:122`）算，並 toast 通知。`startLevel()` 會把旗標重設（`app.ts:409`）

寫檔節流：只有 `metaDirty` 時才倒數 `saveTimer`，**最多每 2 秒一次 `saveMeta()`**（`app.ts:184-189`）。局外操作（買升級／道具／改編隊）則走各自的 handler 立即存檔（`app.ts:330-365`）。

### 音效解鎖

瀏覽器自動播放政策：`AudioContext` 必須在使用者手勢後才建立。`app.ts:68-71` 用 `pointerdown` / `keydown` 各掛一個 `{ once: true }` 的 `unlock`；`Audio.unlock()`（`core/audio.ts:133-157`）建 ctx、master gain（0.9）與一段固定種子白噪音 buffer，已建立則只 `resume()` suspended 的 ctx。`toggleMute()`（`app.ts:312-318`）也會先呼叫 `unlock()`——玩家點喇叭本身就是有效手勢。
...
8. **`index.html` 是 DOM 的唯一定義處。** `hud.ts:66-70` / `screens.ts:71-75` / `wish.ts:16-20` 的 `el()` 找不到節點會直接 `throw`，所以刪 HTML 節點會讓整個 App 建構失敗。加畫面要同時：`index.html` 加 `<section class="screen" hidden>`、`ScreenName` 加字面值（`screens.ts:29`）、`Screens.show()` 加 `hidden` 切換與 render 呼叫。
9. **`hidden` 屬性靠 `style.css:23-25` 的 `[hidden] { display: none !important }` 生效**，元件自己的 `display` 會壓過它。新元件用 `hidden` 而不要自己發明 `.is-open`。
10. **心願面板與資訊面板不能同時開**（都占畫面底部）：`select()` 會 `wishPanel.hide()`（`app.ts:443-447`），`openWishPanel()` 會清 `selectedCell`（`app.ts:292-296`）。
11. **`startLevel()` 的重設清單**（`app.ts:405-419`）：`selectedCell` / `mode` / `renownPaid` / `particles.clear()` / `wishPanel.hide()` / `hud.onLevelChanged()` / `renderer.resize()`。新增任何「跨局會殘留」的 app 層欄位都要加進這裡。
12. **`newSeed()` 是 `Date.now()`**（`app.ts:511`）。要重現 bug 就把它改成固定值——這是唯一的隨機來源入口，`sim/` 內禁用 `Math.random()`（`particles.ts:27-33` 與 `audio.ts:151-154` 也都用固定種子的 LCG，讓畫面／音色在同機器上可重現）。
...
15. **`FX_COLOR['none'] = 'transparent'`**（`fx.ts:23`）：圖鑑／編隊／心願面板拿 fx 當字色時必須排除 `none`，否則字會消失（`screens.ts:463`、`screens.ts:317`、`wish.ts:63` 都寫了 `g.fx !== 'none'` 的 fallback）。
16. **`drainEvents()` 依賴當幀 `state` 仍與事件一致**：`merge` 事件靠 `char` 反查場上單位取座標（`app.ts:227`），找不到就不噴粒子。不要把 drain 延後到下一幀。

## 我想改 X → 動哪裡

| 想改什麼 | 動哪裡 | 注意 |
|---|---|---|
| 新增一種 sim 事件的音效／粒子 | `sim/types.ts:282` 加 `SimEvent` 變體 → sim 內 `emit()` → `app.ts:220` 的 switch | switch 沒 `default`，漏接是靜默的；粒子要新方法就加在 `particles.ts` |
...
| 新增一種攻擊特效 `FxKind` | `sim/types.ts` 的 `FxKind` → `render/fx.ts:13` 的 `FX_COLOR` → `fx.ts:56` 的 switch → `app.ts:516` 的 `attackSfx()` | 三處都要加，`FX_COLOR` 漏了會 undefined 導致整幀繪製異常 |
...
| 加一個全螢幕畫面 | `index.html` 加 `.screen` → `ScreenName`（`screens.ts:29`）→ `Screens` 加 DOM 欄位、`show()` 的 `hidden` 切換與 `renderXxx()` → 需要動 meta 就在 `ScreensHost` 加方法 | 畫面開著時模擬會凍結（`app.ts:403`），這是刻意的 |
| 改選關卡／圖鑑／兵書／商城／編隊的呈現 | `ui/screens.ts` 對應 `renderXxx()` | 資料表本身在 `data/`（見 [02-data-tables.md](../02-data-tables.md)） |
| 改 UI 整體大小或斷點 | `app.ts:114` 的除數（27 / 56）與 clamp（9.5 / 18）、`app.ts:117` 的 320px | 改除數會同時影響所有 DOM 尺寸 |
| 極窄視窗要隱藏更多東西 | `style.css:78` 的 `#app[data-compact='true']` 區塊 | 不要改成 media query：斷點看的是 `#app` 而非視窗（`#app` 有 `max-width: 560px`） |
| 改倍速選項 | `app.ts:34` 的 `SPEEDS` | 倍速只是往累加器多灌時間，太高會頂到 `MAX_STEPS_PER_FRAME` 而變慢動作 |
...
| 加開發密技 | `core/devtools.ts` 加函式 → `ScreensHost`（`screens.ts:42-49`）加方法 → `App` 實作 → `screens.ts:244-251` 的 actions 陣列 | 密技繞過 `sim/actions.ts` 的驗證，僅供測試 |

...
> `app.ts:25`（`recruit` / `rerollHand` / `smelt` / `sellGlyph` / `startWaveNow` / `toggleWish`）、
...
不回傳 JSX**。音效與粒子一律走 `emit()` 事件佇列（不變式 #7），由 `app.ts:220` 每幀 drain。
...
| 欄位 | 意義 | 誰在讀 |
|---|---|---|
| `ok` | 操作是否成立 | `input/pointer.ts:106` 決定要不要 `select(cell)`；`app.ts:302` 決定播 `ui` 還是 `deny` |
| `msg` | 給玩家看的字串（成功摘要或失敗原因） | 全部呼叫端都是 `if (res.msg) toast(res.msg)` |
| `combined` | 這次成的武將名，**可能兩個**（十字同時成兩將） | `input/pointer.ts:105,202` → `host.onCombined()` → `app.ts:457` |
...
`makeGeneralUnit()` 直接把 `parts.map(p => p.cells[0])` 當成 `Unit.cells`（`src/sim/state.ts:312`），
...
3. `delete state.bondCds[name]`（`:264`）——羈絆技冷卻以武將名為 key，武將沒了就得歸零，
   否則重新組同一名武將會繼承舊冷卻
4. 每個被解除的武將 `emit({ kind:'dissolve', name })`（`:265`）→ `app.ts:238` 播音效
...
- **`id` 必須保留**：武將透過 `memberIds` → id 找成員（`glyphsOf()`，`src/sim/state.ts:199`），
  換 id 等於成員憑空消失，`recomputeForm()` 會因為 `parts.length === 0` 直接 return（`state.ts:359`），
  武將屬性靜默凍結在舊值。
...
呼叫端只有 `recalcUnits()` 尾端（`src/sim/state.ts:433`），寫進 `state.hints`；
`state.hints` 接著餵給 `computeHintCells()`（`state.ts:441`）算出棋盤上的 `hintCells` 標記。
...
（例如 `app.ts:302`）。加事件前先確認 `SimEvent` 聯集（`sim/types.ts`）與 `app.ts:220` 的 drain switch
都要同步，否則事件被無聲丟棄。
...
| 提示要提示什麼 | `possibleRecipes()` `combine.ts:138`（名單）＋ `state.ts:441` `computeHintCells()`（棋盤標記） | 純衍生值，不影響機制。`usesHand` 過濾是刻意的 |

...
- **`id` 必須保留**：武將透過 `memberIds` → id 找成員（`glyphsOf()`，`src/sim/state.ts:205`），
  換 id 等於成員憑空消失，`recomputeForm()` 會因為 `parts.length === 0` 直接 return（`state.ts:365`），
  武將屬性靜默凍結在舊值。
...
呼叫端只有 `recalcUnits()` 尾端（`src/sim/state.ts:439`），寫進 `state.hints`；
`state.hints` 接著餵給 `computeHintCells()`（`state.ts:447`）算出棋盤上的 `hintCells` 標記。
...
- `smelt` 的 `state.smeltFreeLeft`（初始 3，`state.ts:174`）不是「免費次數」而是
...
| 提示要提示什麼 | `possibleRecipes()` `combine.ts:138`（名單）＋ `state.ts:447` `computeHintCells()`（棋盤標記） | 純衍生值，不影響機制。`usesHand` 過濾是刻意的 |

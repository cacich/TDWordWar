...
> `app.ts:24`（`recruit` / `rerollHand` / `smelt` / `sellGlyph` / `startWaveNow` / `toggleWish`）、
...
不回傳 JSX**。音效與粒子一律走 `emit()` 事件佇列（不變式 #7），由 `app.ts:182` 每幀 drain。
...
| 欄位 | 意義 | 誰在讀 |
|---|---|---|
| `ok` | 操作是否成立 | `input/pointer.ts:106` 決定要不要 `select(cell)`；`app.ts:264` 決定播 `ui` 還是 `deny` |
| `msg` | 給玩家看的字串（成功摘要或失敗原因） | 全部呼叫端都是 `if (res.msg) toast(res.msg)` |
| `combined` | 這次成的武將名，**可能兩個**（十字同時成兩將） | `input/pointer.ts:105,202` → `host.onCombined()` → `app.ts:410` |
...
3. `delete state.bondCds[name]`（`:264`）——羈絆技冷卻以武將名為 key，武將沒了就得歸零，
   否則重新組同一名武將會繼承舊冷卻
4. 每個被解除的武將 `emit({ kind:'dissolve', name })`（`:265`）→ `app.ts:200` 播音效
...
（例如 `app.ts:264`）。加事件前先確認 `SimEvent` 聯集（`sim/types.ts`）與 `app.ts:182` 的 drain switch
都要同步，否則事件被無聲丟棄。

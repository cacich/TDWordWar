...
> | 檔案 | 規模 | 職責 |
> |---|---|---|
> | `src/core/save.ts` | 138 行 | localStorage 讀寫、版本遷移、載入時的資料清洗 |
...
> `ui/screens.ts`（兵書／商城／編隊／成就／密技五個畫面）、`sim/state.ts:139`（`perksFrom`）、
...
| 系統 | 存哪裡 | 怎麼影響對局 | 檔案 |
|---|---|---|---|
| 圖鑑 | `seenGlyphs` / `seenGenerals` | 不影響（純收集紀錄），但是編隊的解鎖判定來源 | 由 `app.ts:125-156` 每幀寫入 |
...
**⚠ 效果只在「下一局開始」生效**（`ui/screens.ts:230` 的說明文字就是這件事），
因為 `perksFrom` 只在 `createGame`（`sim/state.ts:139`）被呼叫過一次。
...
| `meteorInterval` | 0 | `meteor` 流星火雨 | `sim/step.ts:46`（`<= 0` 直接 return）、`:49`；初值寫進 `sim/state.ts:178` 的 `meteorTimer` |
...
| `atkMul` | 1 | `banner` 號令旗 | `sim/state.ts:401`（`recalcUnits`，乘在羈絆 `atkMul` 之後） |
| `apsMul` | 1 | `gale` 疾風令 | `sim/state.ts:402` |
...
| `rangeMul` | 1 | `range` 精工兵器 | `sim/state.ts:404`（乘在 `effectiveRange` 之後） |
| `cdMul` | 1 | `bondcd` 兵法傳承 | `sim/state.ts:397` → 寫入 `state.cdMul`，再由 `sim/state.ts:426`（`skillCdMax`）與 `sim/bonds.ts:47`／`:66`（組合技 `cdMax`）讀取 |
...
公式在 `sim/state.ts:122-124`（放在 sim 是為了讓測試不必碰 app 層）：

```ts
renownFor(wave, kills, won) = max(1, round(wave * 1.5 + kills * 0.05) + (won ? 20 : 0))
```

波次是主要來源，擊殺是零頭，通關 +20。呼叫點只有 `app.ts:163`，
...
⚠ `totals` **只在一局真正結束（勝或敗）時累加一次**，就寫在 `app.ts:165-170` 的
...
| 時機 | 位置 | 為什麼需要它 |
|---|---|---|
| 每 0.5 秒輪詢 | `app.ts:176-181` | 24 個成就要掃全場單位，不必每幀跑；0.5 秒對 toast 來說夠即時 |
| 一局結束的當下 | `app.ts:173`（`renownPaid` 區塊內） | 「通關且沒掉命」這類條件**只有那一刻成立**，等下一次輪詢玩家可能已經回選單 |
| 切換畫面時 | `app.ts:398-403` 的 `show()` | `syncProgress` 在選單畫面會早退，所以「在選單裡才達成」的成就（把兵書買滿）只能靠這裡補 |

⚠ **成就解鎖是立即 `saveMeta`，不走 2 秒節流**（`app.ts:196-209`）：
多數成就在一局結束的那一刻達成，而玩家常常馬上回選單，等節流會來不及寫入。

⚠ **一次解鎖多項時要合併成一則 toast**。`hud.toast()` 只有一格、後蓋前（`ui/hud.ts:172-176`），
在迴圈裡逐項 toast 只會看到最後一項。`app.ts:204-209` 因此把它們併成一句。
...
`saveMeta` 本身沒有節流，**節流在 app 層**：`app.ts:183-189`，
`metaDirty` 旗標 + `saveTimer` 倒數，最多每 2 秒寫一次（圖鑑是每幀掃描的，不節流會每幀寫 localStorage）。
但**購買／編隊／密技類操作是立即寫入**（`app.ts:333`、`:296`、`:305`、`:311`、`:318`、`:326`、`:347`）——
它們發生在選單畫面，`syncProgress` 因為 `screens.visible` 早退（`app.ts:126`）不會跑到節流那段。
...
buyUpgrade(meta, key)  /  buyItem(meta, key)
  ...
  → 回傳 BuyResult { ok, msg }（msg 直接進 toast，見 app.ts:335）
```
...
```
meta.loadoutActive ? { glyphs, generals, seenGlyphs } : undefined   // sim/state.ts:134-136
  → buildGlyphPool(rng, level, loadout)                             // sim/state.ts:137
...
**⚠ `MAX_LOADOUT_GENERALS = 5` 是新增羈絆的硬約束**（宣告與理由在 `sim/state.ts:108-119`）。
...
它刻意接收兩個陣列而不是整個 `MetaProgress`，這樣 `core/save.ts:70` 才能在物件還沒組完時就呼叫。
...
**⚠ `EMPTY_META`（`core/save.ts:23-39`）與 `DEFAULT_META`（`sim/state.ts:88-104`）是兩份重複定義。**
...
不要以為可以把 `MAX_LOADOUT_GENERALS` 搬進 `data/`——它跟 `MAX_HAND_SIZE` 等一起住在 `sim/state.ts:105-119`。

**只存局外 meta，不存局內狀態。** `GameState` 含 `rng` 閉包，`JSON.stringify` 會靜默丟掉它。
**但**：`mulberry32`（`core/rng.ts:5-14`）的內部狀態只是**單一個 uint32 `a`**，
所以「局內續玩」的成本遠低於舊文件說的「存 seed + 重播整局」——只要讓 rng 額外暴露／接受那個 uint32
（例如回傳 `{ next, getState, setState }`）就能精確續存。擴充方向見 [../06-roadmap.md](../06-roadmap.md)。

**靜音設定不在這裡。** `tdwordwar.muted` 是獨立的 key，讀寫在 `app.ts:533-549`，不走 `MetaProgress`。
...
入口：選單標題 **2.5 秒內連點 7 下**（`ui/screens.ts:123-132` 的 `handleTitleTap`），面板在 `:165-206`。
...
| 改聲望給多少 | `sim/state.ts:122-124` 的 `renownFor` | 商城／兵書總價的「換算局數」註解要跟著改（`shop.ts:12`、`upgrades.ts:5`） |
| 改編隊上限 | `sim/state.ts:107`／`:90` | 動 `MAX_LOADOUT_GENERALS` 前先讀 `loadout.test.ts:23-51`；調小的話舊存檔會被 `loadMeta` 的 `slice` 截斷（可接受） |
| 改「哪些字能選進編隊」 | `data/loadout.ts:22-25` 的 `isLoadoutableGlyph` | 規則改嚴 → 舊存檔下次載入自動清理（`core/save.ts:67`），不用寫遷移 |
| 改武將解鎖判定 | `data/loadout.ts:33-41` 的 `isGeneralUnlocked` | 同時影響圖鑑列表（`ui/screens.ts:337`）與 `loadMeta` 過濾 |
...
| 成就的檢查頻率 | `app.ts:178` 的 `achieveTimer = 0.5` | 調小會讓每幀成本上升（要掃全場單位），調大會讓 toast 變遲鈍 |
| 跨局統計要多記一項 | `RunTotals`（`sim/state.ts:29-40`）＋ `EMPTY_TOTALS` ＋ `core/save.ts` 的 `totals()` ＋ `app.ts:165-170` 的累加 | 累加**只能**放在 `renownPaid` 區塊裡，否則同一局會被重複計入 |
| 破壞性改存檔格式 | `core/save.ts:20-21`：`KEY` 升到 v4，把 v3 推進 `LEGACY_KEYS` | 舊 key 的資料會被當成 partial 解析，不相容的欄位靠 clamp／過濾吸收 |
| 新增一個開發密技 | `core/devtools.ts` 加函式 → `ui/screens.ts:43-49` 的 host 介面 → `app.ts:368+` 轉接 → `:173` 的 `actions` 陣列加按鈕 | 改 `units`／`hand` 要 `recalcUnits`；改 `meta` 要 `saveMeta` |

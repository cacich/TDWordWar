...
> | 檔案 | 規模 | 職責 |
> |---|---|---|
> | `src/sim/step.ts` | 245 行 | `stepGame` 主迴圈：生成／狀態／敵方支援／移動／戰鬥／技能／流星／清屍與死亡分裂／波次結算 |
> | `src/sim/waves.ts` | 141 行 | 血量曲線（`HP_GROWTH`）、數量與其上限、`minWave` 開放、`bias` 加權、`pickBoss`、`buildWave` |
...
> `ui/screens.ts:569`（`countersFor(level.bias)` 推導關卡卡片的「建議帶」標籤）。
...
| `WAVE_REF` | **40** | `waves.ts:51` | 難度弧的參考長度。血量指數 = `wave × WAVE_REF / maxWave`，**短關卡因此把同一條弧壓得更陡**；`maxWave` 非有限（無盡）時它就是實際弧長 |
...
| `gap` | 0.75 秒 | `waves.ts:118` | 出怪間隔，**寫死在 `buildWave` 內的區域變數**，不是匯出常數 |
...
| `level.maxWave` | 12～40 或 `Infinity` | `data/levels/index.ts` | **同時是關卡長度與難度弧的陡度**（見 `WAVE_REF`）。改短一關 = 把同一條弧壓陡，不是只是少打幾波。`Infinity` = 無盡變體，弧長退回 `WAVE_REF` |
| `level.bias` | `EnemyTrait[]` | `data/levels/index.ts:54` | 關卡偏好；同時決定敵種比重與 UI 的推薦標籤 |
...
血量永遠停在第 0 波。`enemyBaseHp` 因此對非有限的 `maxWave` 退回 `WAVE_REF`（`waves.ts:62`），
無盡走的就是 `32 × 1.23^wave` 這條原始曲線。`endless.test.ts` 有測試鎖住這件事
...
一般兵 `damage` 全部是 1。**開放時程**（`eligible()`，`waves.ts:83-87`）：
...
**BOSS 候選數隨波次成長**（`pickBoss`，`waves.ts:102-105`）：
...
`LevelDef.bias: EnemyTrait[]`（`data/levels/index.ts:46-54`）→ `createGame` 抄進 `state.bias`（`state.ts:160`）
→ `beginBattle` 傳給 `buildWave` 第 4 參數（`actions.ts:300`）。加權在 `weightOf`（`waves.ts:89-91`）：
...
推導出關卡卡片上的「建議帶」標籤（`countersFor()`，`enemies.ts:179-185`；顯示在 `ui/screens.ts:569-570`）：
...
| 每波固定收入 | `4 + ⌊wave×0.6⌋`，再 ×`incomeMul` | `economy.ts:60-65`／`step.ts:220` |
...
`recruitsThisWave` 在 `checkWaveEnd`（`step.ts:242`）歸零，所以「同一波連續征兵」會越來越貴（每次 +`RECRUIT_STEP`＝3），跨波則重置。
...
| 15 | 38 | `checkWaveEnd`（`step.ts:209-245`） | **必須在 `cleanupDead` 之後**：條件是 `spawnQueue.length === 0 && enemies.length === 0`，屍體沒清掉波次永遠不會結束；分裂出的小怪也是在這之前才進場，否則波次會提早結束 |
...
| 護衛 | `escort: { key, count }` | `buildWave`（`waves.ts:130-137`），**生成期展開，不是 runtime** |
...
**5. `escort` 是生成期展開，不是 runtime 行為**（`waves.ts:130-137`）。護衛只是被塞進 `spawnQueue` 的普通條目，
...
### `checkWaveEnd`（`step.ts:209-245`）的結算順序
...
**為什麼需要第 0 步**（`step.ts:216`，2026-07-30 修）：`moveEnemies` 在同一幀就可能把 phase 設成
...
`perks.meteorInterval <= 0` 直接 return（未購買時零成本）。倒數 `meteorTimer` 初值＝間隔（`state.ts:193`），
...
2. **`buildWave` 會消耗 `rng`：每波 `enemyCount(wave)` 抽（每隻敵人一抽，`waves.ts:121`），BOSS 波再多 1 抽（`pickBoss`，`waves.ts:126`）**。
...
5. **已成為武將成員的字牌不重複計算**。同一條規則散落**三處**，任何新增的「逐單位聚合」都要跳過 `u.kind === 'glyph' && u.formIds.length > 0`，
   ...
   - 光環投射：`src/sim/state.ts:424`

   第四個聚合（例如「全場攻擊力總和」的 UI、或新的每波結算項目）必須自己加上同樣的判斷。

6. **敵種開放由各敵人的 `minWave` 決定，不是硬編碼的時程表**（`eligible()`，`waves.ts:83-87`）。
   `composition(wave)` 現在只是 `eligible(REGULARS, wave)` 的薄包裝（`waves.ts:97-99`），
   ...
   **所以新增敵種只要改 `data/enemies.ts`，不需要動 `waves.ts`**（這與擴充前的規則相反，舊文件說要改兩處）。
   ...
   - `eligible()` 有一道防呆：全部被 `minWave` 擋掉時退回 `pool[0]`（`waves.ts:86`）。
...
8. **`level.hpMul` 只在兩個地方被乘**：`buildWave`（`waves.ts:115`）與 `stepMeteor`（`step.ts:56`）。
...
10. **perks 的介入點清單**（`Perks` 定義在 `types.ts:349`，由 `data/shop.ts` 的 `perksFrom()` 推導，`createGame` 注入一次、整局固定）。
    ...
    | `incomeMul` | `checkWaveEnd`（`step.ts:220`），**只影響固定收入、不影響產糧** | 1 |
    | `healEveryWaves` | `checkWaveEnd`（`step.ts:232-239`） | 0 |
    ...
    | `extraLives` | `createGame`（`state.ts:174-175`） | 0 |
...
12. `checkWaveEnd` 的通關判定在回血之前（`step.ts:227-231`），所以**通關那一波不會觸發杏林春暖**；
...
| 敵人數量／出怪節奏 | `enemyCount`（`waves.ts:74-76`）、`gap`（`waves.ts:118`） | `gap` 是區域變數；縮小它會同時縮短整波長度與玩家的反應窗口 |
...
| **調整關卡偏好** | `data/levels/index.ts` 的 `bias` | 同時改變敵種比重**與**關卡卡片的「建議帶」標籤（`ui/screens.ts:569`）——不要另外手寫推薦清單 |
...
| BOSS 護衛 | `escort`（`data/enemies.ts`） | 在 `buildWave` 展開（`waves.ts:130-137`），不消耗 rng、**不受 `minWave` 限制** |
...
| 每波收入／產糧 | `waveIncome`（`economy.ts:60-65`）、字表的 `income`（`data/glyphs.ts`）、`屯田` 的 `income`（`generals.ts:228`） | 經濟字產出是 `income × 品質階級`（線性，`state.ts:251`），不是指數 |

...
npm test         # 250 個單元測試
npm run typecheck
npm run sim 12 all # 難度曲線：九關的陣亡中位數與「比例」（比例要一路遞減）
...
各關傻 AI 陣亡中位數基準（目標由該關 `arc` 換算 ≈ `maxWave × 20 / arc`，±20% 內算達標）：
黃巾 12／12・董卓 15／18・巨鹿 24／30・官渡 17／24・赤壁 19／30・五丈原 24／40・
襄陽 17／32・漢中 15／32・洛陽 18／40 —— **比例 1.00 → 0.45 一路遞減，那就是難度曲線**。
...
- 2026-07-31 **難度曲線重做 + 省電**。兩件事其實無關，但都來自同一份回報（「玩久會燙、第一關太難」）：
  1. **難度**：新增 `level.arc`（難度弧長度），把「關卡長度」與「難度」拆成兩個旋鈕。
     以前弧長寫死 `WAVE_REF`，於是每關都走完整條弧、傻 AI 在九關都死在正中間——**難度曲線是平的**，
     而 12 波的教學關被壓成每波血量 ×1.99（全遊戲最陡）。現在九關 arc 20 → 41，抵達比例 1.00 → 0.45。
     順手修掉黃巾之亂的地圖：`C` 在右下角但最後一段路往左走，最後一列有 8 格死路（`mapgen.test.ts` 已加守護）。
  2. **省電**：AI 決策加三層純快取（`Cov`／`Geom.runs`／`defInfo`，每輪成本降 6 成、行為不變）；
     繪製節流到 ≈60fps（120Hz 手機少畫一半）、地形改離屏快取、提示光暈拿掉 `shadowBlur`、
     HUD 全面改成「值變了才寫 DOM」、圖鑑登錄從每幀改成 0.5 秒輪詢。新增 `npm run perf` 當量表。
